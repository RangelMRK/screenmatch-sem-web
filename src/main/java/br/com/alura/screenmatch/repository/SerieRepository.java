...
public interface SerieRepository extends JpaRepository<Serie, Long> {
    ...
    List<Serie> findByNumTemporadasLessThanEqualAndAvaliacaoGreaterThanEqual(Integer temporadas, Double avaliacao);

    @Query("SELECT s FROM Serie s WHERE s.numTemporadas <= :temporadas AND s.avaliacao >= :avaliacao")
    ...
    List<Serie> encontrarEpsMaisRecentes();

    @Query("SELECT e FROM Serie s JOIN s.episodios e WHERE s.id = :id AND e.temporada = :numero")
    List<Episodio> obterTemporadasPorNumero(Long id, Long numero);
